...
// display the background and the points of the lidar 显示背景和lidar点云
...
int main(int argc, char **argv) {
  ...
  // init braitenberg coefficient 初始化系数
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    // get lidar values
    const float *lms291_values = wb_lidar_get_range_image(lms291); //重点！！！

    // apply the braitenberg coefficients on the resulted values of the lms291
    double obstacle = 0.0;
    for (i = 0; i < lms291_width; i++)
      obstacle += braitenberg_coefficients[i] * (1.0 - lms291_values[i] / max_range); //重点！！！

    // compute the speed and the direction according to the information about 根据障碍物信息计算速度和方向
    // a front obstacle
