      ...
      if (autodrive && has_camera) {
        ...
        if (enable_collision_avoidance)
          {obstacle_angle = process_sick_data(sick_data, &obstacle_dist);
           printf("x=%f, y=%f, z=%f, layer_id=%d, time=%f\n",point_cloud->x,point_cloud->y,point_cloud->z,point_cloud->layer_id,point_cloud->time);}
